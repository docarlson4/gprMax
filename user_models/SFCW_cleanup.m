...
Ts = trace.header.dt;
Ns = trace.header.iterations;
RFTime = (0.5:Ns - 0.5)'*Ts;
Fs = 1/Ts;
vel = c0/sqrt(6);
...
figure
plot(RFTime/ns, Pulse)
xlabel('\bf Time (ns) ')
ylabel('\bf Amplitude ')
title('\bf\fontsize{14} Gaussian Dot Norm Pulse ')
xlim([0,5])
...
xlabel('\bf Time (ns) ')
ylabel('\bf Amplitude ')
title('\bf\fontsize{14} Scene Impulse Response ')

% Obtain convolved response
PGPRC = filter(RFImpulse, 1, Pulse);
figure
plot(RFTime/ns, PGPRC)
xlabel('\bf Time (ns) ')
ylabel('\bf Amplitude ')
title('\bf\fontsize{14} Scene Gaussian Dot Norm Response ')
xlim([0,10])
...
for k = 1:Np
    ...
    OSC(Ramp(k)+1:Ns, k) = sin(2*pi*f(k)*RFTime(Ramp(k)+1:Ns));
    OSC_90(1:Ramp(k), k) = 0.25*f(k)*RFTime(1:Ramp(k)).*cos(2*pi*f(k)*RFTime(1:Ramp(k)));
    OSC_90(Ramp(k)+1:Ns, k) = cos(2*pi*f(k)*RFTime(Ramp(k)+1:Ns));
...
SF = I(round(Ns/2),:) + 1j*Q(round(Ns/2),:);
...
xlabel('\bf Time (ns) ')
ylabel('\bf Amplitude ')
title('\bf\fontsize{14} Scene SFCW Response ')
xlim([0,10])
